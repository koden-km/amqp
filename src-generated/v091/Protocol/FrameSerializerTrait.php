...
trait FrameSerializerTrait
{
    public function serialize(OutgoingFrame $frame)
    {
        if ($frame instanceof HeartbeatFrame) {
            ...
            return "\x01" . pack("n", $frame->frameChannelId) . "\x00\x00\x00\x08\x00\x0a\x00\x33\xce";
            ...
            return "\x01" . pack("n", $frame->frameChannelId) . "\x00\x00\x00\x08\x00\x14\x00\x29\xce";
            ...
            return "\x01" . pack("n", $frame->frameChannelId) . "\x00\x00\x00\x08\x00\x5a\x00\x0a\xce";
        } elseif ($frame instanceof Tx\TxCommitFrame) {
            return "\x01" . pack("n", $frame->frameChannelId) . "\x00\x00\x00\x08\x00\x5a\x00\x14\xce";
        } elseif ($frame instanceof Tx\TxRollbackFrame) {
            return "\x01" . pack("n", $frame->frameChannelId) . "\x00\x00\x00\x08\x00\x5a\x00\x1e\xce";
